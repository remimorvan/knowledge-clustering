...
def topological_sort_string(list_strings):
    ...
    for s1 in list_strings:
        ...
        dependency_reversed[s1] = set([s2 for s2 in list_strings if s1 in s2 and s1 != s2])
...
def quote_maximal_substrings(text, list_strings, interactive=True, print_col=False, size_tab=4):
    """
    ...
    for s1 in list_strings_sorted:
        for match in re.finditer(s1, text_cleaned):
            ...
            if not ignore_position[start]:
                ...
                if start > 0 and text_cleaned[start-1] != '"' and end < len(text_cleaned) - 1 and text_cleaned[end+1] != '"':
                    add_quote_location.append((s1, start, end))
    # Using the pointer, describe where to add quotes in the original text
    add_quote_location_origin = [(s, pointer[i], pointer[j]) for (s, i, j) in add_quote_location]
    if None in [i for (_, i, _) in add_quote_location_origin]+[j for (_, _, j) in add_quote_location_origin]:
